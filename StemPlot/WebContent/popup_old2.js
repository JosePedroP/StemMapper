...
function startuppop() {
	...
	for (var i=0;i<lem;i++) 
	{
		...
		subntypes.push(ntypes[i]);
		submedian.push(median(values));
		subpem.push(num-agregator);
		subnmins.push(nmins[i]);
		subnmaxs.push(nmaxs[i]);
	}
...
function loadsubdata()
{
	...
	for (var i=0;i<lem;i++) 
	{
		...
		cell1.innerHTML = subntypes[i];
		var cell2 = row.insertCell(1);
		cell2.innerHTML = submedian[i];
		var cell3 = row.insertCell(2);
		cell3.innerHTML = subpem[i];
		var cell4 = row.insertCell(3);
		cell4.innerHTML = subnmins[i];
		var cell5 = row.insertCell(4);
		cell5.innerHTML = subnmaxs[i];
	}
}